                              ...
                              return  <tr key={key}>
                                        {
                                          props.td.map((row2,key2)=>{
                                              ...
                                              return <td key={key2} className={row2.value==='saldo_string' || row2.value==='saldo'?'text-right':""}>
                                                      {
                                                        (row2.value!=="events")?<div className={(row2.value==='saldo_string' && row.saldo>0)?'text-rosa ':(row2.value==='saldo_string' && row.saldo<0)?'text-dark':(row2.value==='saldo_string2' && row.saldo2>0)?' text-rosa':(row2.value==='saldo_string2' && row.saldo2<0)?'text-dark':''}><div className={row2.className +row.estatus_string}>{row[row2.value]}</div></div>:<div className="d-flex justify-content-center">
                                                            ...
                                                            return  <div key={key3} className="p-1 " title={row3.label}>
                                                                      {
                                                                        ...
                                                                        { row.tmpl==='Mostrar2'?<>
                                                                            ...
                                                                            <row3.NavLink className="" to={row3.url2+row.reporte_pagos.token+"/"}>
                                                                              {row3.icon}
                                                                            </row3.NavLink>
                                                                          </>
