...
const App=(props)=>{
  ...
  const getInit=()=>{
    ...
    let data_                  =   {}
        ...
        data_.filter           =   filter
        if (props.filter!==undefined && props.filter.estatus!==undefined && props.filter.estatus!=='any') {
          data_.estatus        =   est
        }
      ...
      message:<div className="text-center h5">
                ...
                <div className="row justify-content-center mt-3">
                  <div className="col-3" onClick={()=>{submitChangeEstatus(row); context.setModalShow({show:false}); }}>
                    <div className="btn btn-primary btn-block">Rechazar</div>
                  </div>
                </div>
  ...
  const submitChangeEstatus=(row)=>{
    let data_ = {op_reporte_pagos_id:row.op_reporte_pagos_id}
    Functions.PostAsync("Administracion","setEstatusReportePago",data_,{},{name:"callbackContinue4",funct:callbackContinue4})
  }
                            ...
                            return  <tr key={key}>
                                      {
                                        props.td.map((row2,key2)=>{
                                          return <td key={key2}>
                                                  {
                                                    (row2.value!=="events")?<div className={row2.className+row[row2.value]}>
                                                                              {row2.NavLink===undefined || parseFloat(row[row2.value])<=0?<>
                                                                                {row[row2.value]}
                                                                              </>:<>
                                                                                <row2.NavLink className="" to={row2.to+row[row2.current]}>
                                                                                  {row[row2.value]}
                                                                                </row2.NavLink>
                                                                              </>}
