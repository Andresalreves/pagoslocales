...
const App=(props)=>{
  ...
  const callbackContinue=(response)=>{
    ...
    setDetalle(response.data.bolsillo)
                            ...
                            return  <tr key={key}>
                                      {
                                        props.td.map((row2,key2)=>{
                                          return <td key={key2}>
                                                  {
                                                    (row2.value!=="events")?row[row2.value]:<div className="d-flex justify-content-center">
                                                      {row2.icons.map((row3,key3)=>{
                                                        return  <div key={key3} className="p-1 " title={row3.label}>
                                                                  {
                                                                    row3.event_andres!==undefined&&row3.url===undefined?<>
                                                                    <div>
                                                                      <i className={row3.icon} onClick={PeticionServer} modelo={row3.modelo} metodo={row3.metodo} registro_id={row.id_billetera} />
                                                                    </div>
                                                                    </>:row3.url!==undefined&&row3.event ===undefined?<>
                                                                    {
                                                                      <row3.NavLink className="" to={row3.url+row.cuentas_inscritas_id}>
                                                                        {row3.icon}
                                                                      </row3.NavLink>
                                                                    }
                                                                  </>:<>
