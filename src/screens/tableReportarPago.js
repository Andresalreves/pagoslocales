...
const App=(props)=>{
  ...
  useEffect(() => {
    context.socketIo.on("tableReportarPago", () => {
      getInit()
    });
  }, [context.socketIo]);
  ...
  const getInit=()=>{
    ...
    let data_                  =   {}
        ...
        data_.filter           =   filter
        data_.estatus          =   est
        data_.espera_soporte   =   5
        data_.extra            =   props.extra
    Functions.PostAsync(props.modelo,props.metodo,data_,{skip:true},{name:"callbackContinue",funct:callbackContinue})
  }
                            ...
                            return  <tr key={key}>
                                      {
                                        props.td.map((row2,key2)=>{
                                          return <td key={key2}>
                                                  {
                                                    (row2.value!=="events")?<>
                                                        {row2.extra===undefined?<div className={row2.value==='valor_abonado_string' || row2.value==='valor_string'?"text-right":row2.className!==undefined?row2.className:""}>
                                                            {row[row2.value]}
                                                          </div>:<div className={row.ma_estatus_id==="9"?"text-warning":row.ma_estatus_id==="3"?"text-warning":row.ma_estatus_id==="2"?"text-rosa text-center":row.ma_estatus_id==="5"?"text-warning "+row2.className+row.estatus_string:"text-warning "+row2.className+row.estatus_string}>
                                                            {row[row2.value]}
                                                        </div>}
                                                        ...
                                                        return  <div key={key3} className="p-1 " title={row3.label}>
                                                                  {
                                                                    ...
                                                                    {
                                                                      <row3.NavLink className="" to={row3.url+row.op_reporte_pagos_id}>
                                                                        {row3.icon}
                                                                      </row3.NavLink>
                                                                    }
                                                                  </>:<>
