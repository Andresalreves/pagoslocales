...
const App=()=>{
  ...
  const calcular          = ()=>{
    ...
    //   data__              = {...response}
    //   setData(response)
    //   if (response.porcentaje!==undefined) {
    //     data__            =   response
    //   }
    // }
    //
    // if (data__.porcentaje!==undefined) {
    //     console.log(500);
    //   let por_restar          = parseFloat(data__.valor) * parseFloat(data__.porcentaje.porcentaje_intermediacion)   / 100
    //       setEquivalencia(Functions.format(parseFloat(data__.valor) - por_restar))
    //       setEquivalencia_comision(Functions.format(por_restar))
    //       console.log(por_restar);
    // }
  }
  ...
  useEffect(() => {
    setReset(true);
    let inputs_                   = {...inputs}
        inputs_.valor_transferir  = data.valor
        inputs_.valor_recibido    = data.valor
        setInputs(inputs_)
  },[data.valor])
